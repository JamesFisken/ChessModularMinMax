...
cdef int converttosinglenum(int x, int y):
    return y*8 + x

cdef tuple converttopygameval(int val):
    cdef int x, y
    x = val % 8
    y = val // 8
    return x, y
...
cdef class bot:
    ...
    cdef dict piece_attacking_values
    cdef np.ndarray Pawns
    cdef np.ndarray FlippedPawns
    cdef np.ndarray Knights
    cdef np.ndarray FlippedKnights
    cdef np.ndarray Bishops
    cdef np.ndarray FlippedBishops
    cdef np.ndarray Rooks
    cdef np.ndarray FlippedRooks
    cdef np.ndarray Queens
    cdef np.ndarray FlippedQueens
    cdef np.ndarray Kings
    cdef np.ndarray FlippedKings

    cdef float piece_map_valuation
    cdef float piece_usefulness_valuation

    def __init__(self, bint side):
        self.side = side
    ...
    def minimax(self, board, depth, alpha, beta, maximizingPlayer):
        cdef float max_value, min_value
    ...
    cdef int mappiece(self, str piece, np.ndarray pieceMap, int x, int y):
        if piece.islower():
